...
function draw() {
  ...
  if (!animationComplete) {
    // Apply rotation to the camera for 360 orbit during zoom-in
    cameraRotation += 0.01;  // Slow rotation rate for orbit effect
    let x = cos(cameraRotation) * cameraDistance;
    let z = sin(cameraRotation) * cameraDistance;

    camera(x, 0, z, 0, 0, 0, 0, 1, 0);  // Move the camera around in a circular path
  }
